...
InstrList Commands(cmd command) 
{
  ...
  if (command->kind == E_ATRIB) 
  ...
  else if(command->kind == E_FOR1)
    {
      ...
      // criar o if
      Expr ifcondition = ast_boolean(LESS_THAN, ast_var(startVar->content.var), command->attr.for1.cond);
      cmd ifcommand = astIf(ifcondition, NULL, NULL);
  ...
  /*  else if(command->kind == E_OUTPUT)
    ...
    {
      InstrList list = NULL;
      Pair p = Expressions(command->attr.input);
